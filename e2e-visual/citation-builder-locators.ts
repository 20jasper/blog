...
export function getCitationBuilderLocators(page: Page) {
	return {
		...
		historyPhrase: page.getByLabel('Prior/subsequent history phrase'),
		historyCitation: page.getByLabel('Prior/subsequent history citation'),
		...
		signal: page.getByLabel('Introductory signal'),
		...
		saveButton: page.getByRole('button', {
			name: /^(Save citation|Update saved citation)$/u,
		}),
